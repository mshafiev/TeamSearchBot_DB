...
# Модель для результатов олимпиад
...
class Users(Base):
    ...
    goal = Column(Integer, nullable=True)  # 0-совместный бот, 1-общение, 2-поиск команды, 3-отношения
    who_interested = Column(Integer, nullable=True)  # 0-ж / 1-м / 2-все
    date_of_birth = Column(String, nullable=True)  # дата рождения пользователя (в формате ДД-ММ-ГГГГ)